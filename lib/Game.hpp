...
public:
    ...
    int filter_out_edge_states(std::pair<std::set<size_t>, std::set<size_t>>& winning_region, Template& assump, Template& strat, const bool print_actions=false) const {
        if (labels_.empty()){
            return 0;
        }
        ...
        filter_templates(assump, org_vertices, print_actions);
